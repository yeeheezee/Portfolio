...
{
    public sealed class StunChainEffectHandler : IChainEffectHandler
    {
        public SpellRecipeEffectType EffectType => SpellRecipeEffectType.Stun;
        ...
        public bool TryApply(CastContext context, ChainEffectEntry entry)
        {
            ...
            for (int i = 0; i < hits.Length; i++)
            {
                ...
                {
                    receiver.ApplyStatus(StatusEvent.CreateCrowdControl(CrowdControlType.Stun, duration, 1f, context.Caster));
                    applied++;
                }
            }
            ...
            if (applied > 0)
            {
                Debug.Log($"[ChainRuntime] effect=Stun applied={applied} duration={duration:F2}");
            }
